...
class CartManager {
    constructor() {
        ...
        this.ca// إدارة سلة المشتريات - نسخة مختصرة
...
class CartManager {
    ...
    // إضافة منتج إلى السلة - نسخة مبسطة
    addToCart(productId, category = null) {
        ...
        if (existingItemIndex !== -1) {
            ...
            console.log('زيادة كمية المنتج:', this.cart[existingItemIndex].quantity);
        } else {
            // إضافة منتج جديد
            this.cart.push({
                id: product.id,
    ...
    renderCart() {
        ...
        console.log('عرض السلة، عدد المنتجات:', this.cart.length);
    ...
    updateCartItemQuantity(productId, change) {
        ...
        if (itemIndex !== -1) {
            ...
            if (this.cart[itemIndex].quantity <= 0) {
                ...
                window.uiManager?.showNotification('تمت الإزالة', 
    ...
    updateProductUI(productId) {
        ...
        console.log('تحديث واجهة المنتج:', productId, 'الكمية:', quantity);
        
        // تحديث الكمية في بطاقة المنتج
...
// تهيئة مدير السلة
window.cartManager = new CartManager();rt = this.loadCartSafely();
        this.cartItemsContainer = document.getElementById('cart-items');
        this.cartSubtotal = document.getElementById('cart-subtotal');
        this.cartTotal = document.getElementById('cart-total');
        this.cartCount = document.getElementById('cart-count');
